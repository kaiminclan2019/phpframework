<?php
class model_driver_mysql extends model_driver_db {
	...
	public function exec_sql($sql,$mode){
		...
		if(!$this->hasError()){
			...
			$this->error = mysql_error($this->curlink);
			$this->status = mysql_errno($this->curlink);
			if($this->error){
