...
class helper_sphinxclient
{
	...
	function _Connect ()
	{
		...
		if ( $this->_timeout<=0 )
			$fp = $this->fsocketopen ( $host, $port, $errno, $errstr );
		else
			$fp = $this->fsocketopen ( $host, $port, $errno, $errstr, $this->_timeout );
		if ( !$fp )
		{
			if ( $this->_path )
				$location = $this->_path;
